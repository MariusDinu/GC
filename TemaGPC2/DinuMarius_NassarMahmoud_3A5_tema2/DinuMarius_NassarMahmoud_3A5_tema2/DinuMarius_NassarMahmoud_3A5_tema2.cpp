...
//functia (distanta catre cel mai apropiat intreg)
void Display3()
{
    double ratia = 0.05;
    double xmax = 25;
    ...
    glColor3f(1, 0.1, 0.1); // rosu
    glBegin(GL_LINE_STRIP);
    //pentru fiecare x care se incrementeaza cu ratia pana la xmax ( nr maxim de puncte)
    for (double x = 0; x < xmax; x += ratia)
    {
        if (x > 0)
        {
            double ceil_value = ceil(x) - x; //luam cel mai apropiat numar mai mare sau egal cu x si scadem x ca sa calculam distanta
            double floor_value = x - floor(x);//luam cel mai apropiat numar mai mic sau egal cu x si scadem din x ca sa calculam distanta
            //facem fucntia d(x)/x
        ...
        //pentru scalare uniforma avem punctul de tip x/M unde M este xmax 
        double x1, y1;
        x1 = x / xmax;
        y1 = y-ratia;
...
void Display4()
{
    ...
    glColor3f(1, 0.1, 0.1); // rosu
    glBegin(GL_LINE_STRIP);
    //t este inrte -pi si pi
    for (double t = -pi + ratia; t < pi; t += ratia)
    {
        //folosim formulele date 
